...
export default function AnimatedLessonPage() {
  ...
  return (
    <div className="flex-1 min-h-screen bg-surface dark:bg-background">
      {/* Page Header */}
      <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 dark:from-emerald-900 dark:to-emerald-800 px-6 py-5">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-2 text-emerald-200 text-xs font-label uppercase tracking-widest mb-2">
            ...
            <Link href="/learn/chapter/math-3" className="hover:text-white transition-colors">অধ্যায় ৩</Link>
            <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>chevron_right</span>
            <span>অ্যানিমেটেড পাঠ</span>
          </div>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-emerald-200 text-sm mt-1">অধ্যায় ৩: ভগ্নাংশের খেলা</p>
            </div>
            <button
              onClick={() => setIsNarrating(!isNarrating)}
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold border transition-all shrink-0 ${
                isNarrating
          ...
          <div className="mt-4 flex items-center gap-3">
            <span className="text-emerald-200 text-xs font-label shrink-0">অধ্যায় ৩/১২</span>
            <div className="flex-1 h-2 bg-white/20 rounded-full overflow-hidden">
              <div className="h-full bg-white/80 w-1/4 rounded-full transition-all duration-1000" />
            </div>
            <span className="text-emerald-200 text-xs font-label shrink-0">২৫%</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 pb-32">
        <div className="mb-8">
          <span className="bg-tertiary-fixed text-on-tertiary-fixed px-3 py-1 rounded-full text-xs font-bold font-label tracking-wider uppercase inline-block">
            অধ্যায় ৩: ভগ্নাংশের খেলা
          </span>
          <p className="text-lg text-on-surface-variant leading-relaxed mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-8 bg-surface-container-lowest dark:bg-surface-container rounded-3xl p-6 md:p-8 border border-outline-variant/15 dark:border-emerald-900/20 shadow-sm">
            ...
            <div className="space-y-6 py-2">
              ...
              <div className="space-y-2">
                ...
                <div className="h-12 w-full bg-surface-container-highest dark:bg-surface-container-high rounded-xl overflow-hidden flex">
                  <div
          ...
          {/* Mini Challenge */}
          <div className="md:col-span-12 bg-surface-container-lowest dark:bg-surface-container rounded-3xl p-6 md:p-8 flex flex-col md:flex-row gap-6 items-center border border-outline-variant/20 dark:border-emerald-900/20 shadow-sm">
            <div className="w-full md:w-1/3 aspect-video rounded-2xl overflow-hidden bg-surface-container-low dark:bg-surface-container-high flex items-center justify-center">
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="flex gap-4 flex-wrap">
                ...
                <Link
                  href="/learn"
                  className="px-6 py-2 bg-surface-container-high dark:bg-surface-container text-on-surface-variant font-bold rounded-xl text-sm hover:bg-surface-variant transition-colors"
                >
                  পাঠে ফিরি
                </Link>
              </div>
      ...
      {/* Floating Toolbar — stays above mobile bottom nav */}
      <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 bg-surface-container-lowest/90 dark:bg-surface-container/90 backdrop-blur-xl px-6 py-3 rounded-full shadow-2xl border border-outline-variant/10 dark:border-emerald-900/20 flex items-center gap-4 z-40">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-1.5 rounded-full transition-colors ${
            isNarrating ? 'bg-primary-container text-primary-fixed' : 'bg-surface-container-high dark:bg-surface-container text-on-surface-variant'
          }`}
        >
          <span className="material-symbols-outlined text-sm">headphones</span>
          <span className="text-xs font-bold font-label hidden sm:inline">{isNarrating ? 'Voice Active' : 'Voice Off'}</span>
        </button>
      </div>
    </div>
  );
